        ...
        static void Main(string[] args)
        {
            ...
            if (filePaths.Length > 0)
            {
                ...
                using (History history = History.Load())
                {
                    ...
                    GraphData data = history.ExtractData();
                    XValue[] xValues = data.XValues.ToArray();
                    using (var diagram = new Graph(4096, 2048))
                    {
                        diagram.SetupAndRenderAxes(xValues[0].X, xValues[xValues.Length - 1].X, data.MinY, data.MaxY);
                        foreach (var sequence in data)
                        {
                            diagram.DrawSequence(sequence, xValues);
                        }
                        diagram.WriteToFile();
                    }
                }
